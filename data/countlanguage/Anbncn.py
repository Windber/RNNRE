...
class Anbncn:
    ...
    def __init__(self, V, T, S, P, prob, name, diction):
        self.Grammar = Anbncn.Grammar(V, T, S, P)
        self.prob = prob
        self.name = name
    ...
    def generate(self):
        ...
        pattern = "|".join(self.Grammar.P.keys())
...
#                 ran = "".join(list(map(str, list(ran))))
    ...
    def accept_sub(self, s):
        length = len(s)
        right = "a" * int(length / 3) + "b" * int(length / 3) + "c" * int(length / 3)
        if s == right:
            return True
        else:
            return False
...
propab = [1/8, 1/16]
propab = list(map(lambda x: 1 - x * 1/2, propab))
negmaxlen = 32
anbncn = Anbncn(V=["S", "A", "B"],
            T=["a", "b", "c"],
            S="S",
            P={"S": [["abc"], ["aAbc"]],
               "Ab": [[], ["bA"]],
